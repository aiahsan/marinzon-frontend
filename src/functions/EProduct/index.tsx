...
export function GetEProduct(page?:any,search?:any,showApproved?:boolean,categoryId?:number) {
   return function (dispatch: any, getState: any): any {
    (async () => {
      try {
        dispatch(loadingAction(true));
         const { status, data }: any = await repository
          .GetEProduct(getState().User?.token || "","0",true,page,search,showApproved,categoryId)
          .then((x) => x);
